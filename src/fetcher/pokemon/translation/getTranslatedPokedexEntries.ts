...
function getIndexOf (form : string, array) {
...
async function getFRPokedexEntries ($, keywords, name) {
  ...
  for (let gen = startGen; gen < currentGen; gen++) {
    ...
    elem = $gen(elem).next()
    while ($gen(elem)[0] !== undefined && $gen(elem)[0].name !== 'h2') {
...
export async function getTranslatedPokedexEntries ($ : cheerio.Root, name : Name, keywords) {
